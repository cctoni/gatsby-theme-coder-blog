---
alias: '2758cfc6d427'
heroImage: /blog/posts/graphql-realtime-server.png
authors: [nikolasburk]
---

import { PostTitle, Abstract } from 'gatsby-theme-coder-blog/src/components/Blog/Post/Metadata'

<PostTitle>Tutorial: Building a Realtime GraphQL Server with Subscriptions</PostTitle>

<Abstract>
GraphQL subscriptions allow clients to receive realtime updates from a GraphQL API upon specific events.

Added to the [official GraphQL spec](http://facebook.github.io/graphql/draft/#sec-Subscription) only
about a year ago, GraphQL subscriptions are one of the newer and less explored features of GraphQL. In this
tutorial, you’ll learn how you can easily add subscriptions to your GraphQL API using [Prisma](https://www.prisma.io/) and [Prisma bindings](https://github.com/prismagraphql/prisma-binding).
...
Consider again the sample `updatePost`-mutation from the section [\*\*2.3.3](https://blog.graph.cool/tutorial-building-a-realtime-graphql-server-with-subscriptions-2758cfc6d427#bf6a)\**. But let’s now assume, the subscription query includes *all\* the fields we just discussed:
...
The first step, as usual when adding new features to a GraphQL API, is to express the new operation as a [_root field_](https://blog.graph.cool/graphql-server-basics-the-schema-ac5e2950214e#9e63) in the GraphQL schema.
...
> This is a bit tricky to understand at first. If you have trouble following right now, be sure to check out [this](https://blog.graph.cool/graphql-server-basics-demystifying-the-info-argument-in-graphql-resolvers-6f26249f613a) technical deep-dive about the `info` object and its role within GraphQL resolvers.
